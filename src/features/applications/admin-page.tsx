...
export async function AdminApplicationsPage({ params }: { params: Record<string, string | undefined> }) {
 ...
 const queue = await getAdminApplications(role,bucket,page,q);
 const href = (r = role, b: ApplicationBucket = bucket, n = 0) => `/admin/applications?role=${r}&status=${b}&page=${n}${q ? `&q=${encodeURIComponent(q)}` : ""}`;
 return <section className="applications-page">
  ...
  <ApplicationSearch q={q} />
  <p className="application-count">Заявок: {queue.total} · Время указано по МСК</p>
  {!queue.items.length && <div className="panel application-empty"><h2>{q ? "Ничего не найдено" : "Заявок пока нет"}</h2><p>В этом разделе появятся заявки с выбранным статусом.</p></div>}
