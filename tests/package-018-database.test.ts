...
test("018 migrations, billing authorization, snapshots, private backgrounds and rich media",async t=>{
 ...
 const dir=new URL("../supabase/migrations/",import.meta.url);for(const f of (await readdir(dir)).filter(f=>f.endsWith(".sql")&&!f.includes("018_")&&!f.includes("019_")).sort())await db.exec(await readFile(new URL(f,dir),"utf8"));await db.exec("alter table auth.users disable trigger user");
