...
for(const role of ["student","tutor"])test(`009 moderated application flow and Telegram mock: ${role}`,async({page,request,browser})=>{
 ...
 try {
  ...
  expect(state.apps[0].status).toBe("pending_review");expect(state.messages.filter((m:{text:string})=>m.text.startsWith("Новая заявка")).length).toBe(2);
  expect(state.messages.some((m:{text:string})=>m.text.includes("register?token"))).toBe(false);expect(state.messages.every((m:{hasButtons:boolean})=>!m.hasButtons)).toBe(true);
  ...
  state=await (await request.get(`${fixture}/applications-state`)).json();const link=(state.messages.at(-1).text as string).match(/http:\/\/localhost:3100\/register\?token=[\w-]+/)![0];
  ...
  state=await (await request.get(`${fixture}/applications-state`)).json();const fresh=(state.messages.at(-1).text as string).match(/http:\/\/localhost:3100\/register\?token=[\w-]+/)![0];expect(fresh).not.toBe(link);
