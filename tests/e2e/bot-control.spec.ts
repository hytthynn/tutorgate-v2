...
test("bot starts fresh flows and edits menu steps, preserving teacher messages",async({page,request})=>{
 let update=130000;
 ...
 const callback=async(data:string,message_id?:number)=>post({callback_query:{id:String(update),from:{id:100004},data,message:{message_id:message_id??(await state()).messages.filter((m:{chat_id:string})=>m.chat_id==="100004").at(-1).message_id,chat:{id:100004,type:"private"}}}});
 ...
 expect((await text("/start")).ok()).toBe(true);
 expect((await state()).edits).toHaveLength(0);
 ...
 let s=await state();expect(s.messages.filter((m:{chat_id:string})=>m.chat_id==="100004")).toHaveLength(3);expect(s.edits.length).toBeGreaterThanOrEqual(5);
 expect(s.messages.find((m:{message_id:number})=>m.message_id===panel.message_id).text).toBe(panel.text);
 ...
 s=await state();expect(s.messages.find((m:{message_id:number})=>m.message_id===teacher.message_id).text).toBe(teacher.text);expect(s.messages.at(-1).text).toContain("Вы пишете:");
 await request.post(f+"/behavior",{data:{op:"edit",fail:true}});expect((await callback("chat:cancel")).status()).toBe(503);expect((await state()).messages.length).toBe(s.messages.length);
 expect((await callback("chat:cancel")).ok()).toBe(true);
 const current=(await state()).messages.at(-1);
 await request.post(f+"/telegram/delete-control",{data:{chat_id:"100004",message_id:current.message_id}});
 expect((await callback("chat:choose",current.message_id)).ok()).toBe(true);
 s=await state();expect(s.messages.length).toBeGreaterThan(5);
