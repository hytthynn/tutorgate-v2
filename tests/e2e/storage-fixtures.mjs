...
export function storageFixture(req,res,url,bytes) {
  ...
  const bucket=url.pathname.includes("schedule-backgrounds")?"schedule-backgrounds":"chat-attachments";
  const key=decodeURIComponent(url.pathname.split(bucket+"/")[1]??"");
  if(url.pathname.includes("/upload/sign/")&&req.method==="POST")json({url:`/object/upload/sign/${bucket}/${key}?token=fixture`});
  else if(req.method==="PUT" || (req.method==="POST" && !url.pathname.includes("/sign/"))) {objects.set(key,bytes);json({Key:key});}
  else if(req.method==="POST"&&url.pathname.includes("/object/sign/"))json({signedURL:`/object/sign/${bucket}/${key}?token=fixture`});
