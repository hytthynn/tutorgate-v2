...
test("014 schedule, media, deletion and read contracts",async t=>{
 ...
 await t.test("017 service sync guard and durable multi-file albums",async()=>{
   const service=async<T>(sql:string,args:unknown[]=[])=>{await db.exec("set role service_role");try{return await db.query<T>(sql,args);}finally{await db.exec("reset role");}};
   await db.exec("alter function private.deleting_profile_locked() security invoker");
   await assert.rejects(service("update profiles set telegram_username='updated' where id=$1",[id(4)]),{code:"42501"});
   await db.exec("alter function private.deleting_profile_locked() security definer");
   await service("update profiles set telegram_username='updated' where id=$1",[id(4)]);
   assert.equal((await db.query<{telegram_username:string}>("select telegram_username from profiles where id=$1",[id(4)])).rows[0].telegram_username,"updated");
   await db.query("select chat_bot_set_recipient($1,$2)",[id(4),id(2)]);
   const target=async(group:string,update:number)=>(await service<{v:{status:string;tutor:string}}>("select chat_bot_album_target('Person 4','Person 4',$1,null,$2) v",[group,update])).rows[0].v;
   const file=async(n:number,size=20)=>{const fid=id(n);const path=(await db.query<{v:string}>("select chat_prepare_upload($1,$2,$3,$4,$5) v",[id(2),id(4),fid,`file${n}.png`,size])).rows[0].v;return {id:fid,path,name:`file${n}.png`,size,type:"image/png"};};
   const receive=async(group:string,update:number,f:unknown,text="")=>(await service<{v:{status:string;messageId:string;albumContinuation:boolean}}>("select chat_bot_receive_album('Person 4','Person 4',$1,$2,$3,$4,$5) v",[group,update,text,JSON.stringify(text?[{text,marks:[]}]:[]),JSON.stringify(f)])).rows[0].v;
   assert.equal((await target("album",170001)).status,"ok");
   const first=await receive("album",170001,await file(170001),"Альбом");assert.equal(first.status,"sent");
   assert.equal((await db.query<{n:number}>("select count(*)::int n from private.telegram_chat_state where student_id=$1",[id(4)])).rows[0].n,0);
   assert.equal((await target("album",170002)).tutor,id(2));
   const secondFile=await file(170002);const second=await receive("album",170002,secondFile,"Вторая подпись");
   assert.equal(second.messageId,first.messageId);assert.equal(second.albumContinuation,true);
   assert.equal((await receive("album",170002,secondFile)).status,"duplicate");
   assert.equal((await target("album",170002)).status,"duplicate");
   assert.equal((await target("different-album",170003)).status,"choose");
   assert.equal((await db.query<{n:number}>("select count(*)::int n from chat_attachments where message_id=$1",[first.messageId])).rows[0].n,2);
   assert.equal((await receive("album",170004,await file(170004,10485760))).status,"too_large");
   assert.equal((await db.query<{body:string}>("select body from chat_messages where id=$1",[first.messageId])).rows[0].body,"Альбом\nВторая подпись");
   await assert.rejects(as(4,()=>db.query("select chat_bot_album_target('Person 4','Person 4','album',null,17)")),{code:"42501"});
   await db.query("delete from chat_conversations where student_id=$1 and tutor_id=$2",[id(4),id(2)]);
   assert.equal((await db.query<{n:number}>("select count(*)::int n from private.telegram_albums where student_id=$1",[id(4)])).rows[0].n,0);
   await db.query("delete from private.chat_uploads where id=$1",[id(170004)]);
 });
