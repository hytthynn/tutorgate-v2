...
## Сценарий после пакета 015

Обычный `/start`: «Открыть TutorGate», а для ученика дополнительно «Написать репетитору». Последняя кнопка обновляет текущую панель на список преподавателей и «Назад», даже если преподаватель один. Выбор открывает ввод, «Отмена» возвращает список. «Главное меню» сбрасывает адресата.

После сохранения сообщения ученика бот отправляет квитанцию с его текстом, кнопками «Отправить ещё» и «Главное меню», затем удаляет сообщение ввода и предыдущую панель. Ошибка записи в БД сохраняет ввод. Новый callback `chat:reply:<message UUID>` проверяется по сохранённой Telegram mapping: после ответа исходное текстовое сообщение преподавателя обновляется с текстом преподавателя и ответом ученика, служебная панель и ответ ученика удаляются. Native Reply продолжает работать. При недоступном для редактирования сообщении квитанция отправляется отдельно. Старые `chat:to` остаются совместимым выбором преподавателя.

Для длинных сообщений квитанция сокращает отображаемые части, чтобы поместить обе стороны в лимит Telegram; полный текст остаётся в web-чате и исходных частях доставки. Внешние действия не могут быть общей транзакцией с БД: при ошибке доставки квитанции пользовательский ввод не удаляется, повтор webhook не дублирует запись. Удаление старых сообщений может быть отклонено Telegram — действуют [ограничения Bot API](https://core.telegram.org/bots/api#deletemessage). Эти операции не удаляют сохранённую переписку на сайте.
...
## Единое сообщение управления

Команды /start (включая подтверждение по токену) и ответы на текст или вложение пользователя создают новое сообщение внизу чата. Кнопки текущей панели — picker, страницы списка, выбор получателя и отмена — обновляют её через [editMessageText](https://core.telegram.org/bots/api#editmessagetext). Кнопка из старой панели или сообщения преподавателя начинает новую панель. Преподавательские сообщения, уведомления и отдельные защищённые ссылки регистрации/сброса доставляются самостоятельно.

Редактируется только ID из private.telegram_control_messages при совпадении с сообщением нажатой кнопки. Поэтому «Ответить» под текстом преподавателя не изменяет его сообщение и не нарушает native Reply. Повторная идентичная правка — успешный no-op. При отсутствии сохранённого ID или ответе Telegram «message to edit not found» панель восстанавливается; timeout, 429, 5xx при редактировании не вызывают дополнительную отправку. Старые сообщения бота не удаляются.

При сетевой неопределённости между первой отправкой и сохранением её ID невозможно обещать exactly-once внешний эффект. Lease предотвращает немедленное повторное создание; после аварии до сохранения ID может понадобиться новая панель. Содержимое меню и Telegram IDs не возвращаются веб-клиенту.


## Уточнения 016

В `/start` для всех ролей добавлена «Поддержка» → `https://t.me/tutorgate`. В каталогах администратора при наличии username открывается `t.me/username`; без username кнопка ведёт в бота с `start=contact_<profile UUID>`. После проверки активной роли admin бот показывает inline-кнопку профиля `tg://user?id=…`. Прямая ID-ссылка с веб-сайта не используется: [Telegram документирует её как абстракцию Bot API](https://core.telegram.org/api/links#id-links). Доступность самого профиля определяется клиентом и настройками Telegram; внешние аккаунты в локальных проверках не используются.
