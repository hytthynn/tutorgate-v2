...
Telegram передаёт альбом отдельными updates с общим `media_group_id`. Webhook сохраняет этот идентификатор; привязка к преподавателю хранится в БД, поэтому следующие части не зависят от уже сброшенного recipient state или процесса сервера. Файлы постепенно дополняют одно сообщение на сайте, повторные updates игнорируются. Подписи объединяются, квитанция дополняется списком файлов; исходные Telegram сообщения удаляются только после сохранения и подтверждения. Уведомление преподавателю отправляется при первой части. При превышении общего лимита уже принятые части сохраняются, неподходящая часть отклоняется с сообщением об ошибке.

Синхронизация учитывает `username` и резервный `active_usernames[0]`; ошибки API/сохранения показываются как частичный результат, а не безусловный успех. Исправление прав триггера требует миграцию 017.
