...
По дополнительному запросу после 012 меню, picker, страницы списка, выбор получателя, отмена, валидация и подтверждение отправки обновляют одну постоянную панель через [editMessageText](https://core.telegram.org/bots/api#editmessagetext). Подтверждение Telegram по /start token также обновляет панель. Преподавательские сообщения, уведомления и отдельные защищённые ссылки регистрации/сброса продолжают доставляться самостоятельно.

Редактируется только ID из private.telegram_control_messages, а не message_id нажатой кнопки. Поэтому «Ответить» под текстом преподавателя не изменяет его сообщение и не нарушает native Reply. Повторная идентичная правка — успешный no-op. Новая панель создаётся при отсутствии сохранённого ID или ответе Telegram «message to edit not found»; timeout, 429, 5xx и прочие ошибки не вызывают дополнительную отправку. Уже существующие старые сообщения бота не удаляются.
