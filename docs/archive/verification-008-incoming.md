# Исторический отчёт из входного архива

Ниже сохранён отчёт предыдущего исполнителя. Эти проверки НЕ являются проверками пакета 009.
...
Предыдущий итоговый отчёт 007 упоминался в исходнике, но не входил в присланный архив. Старые логи в verification-logs и материалы verification-ui относятся к прошлому пакету.

## Повторное применение SQL после 40P01

Добавлен генератор `scripts/prepare-schedule-migration.mjs`: существующая миграция 007 не изменяется, отдельный SQL-файл получает nonblocking advisory/table lock preflight перед DDL. Выполнены `npx tsx --test tests/migration-retry.test.ts tests/schedule-features-database.test.ts` — 10/10, `npm run typecheck` и `npm run lint`. PGlite проверяет применение обёрнутой миграции и бизнес-инварианты; полноценное конкурентное воспроизведение двух PostgreSQL backend-сессий этим прогоном не выполнялось. К рабочей Supabase скрипт не применялся. По OID из сообщения нельзя установить имена таблиц без доступа к каталогу конкретной БД; участие Cron предполагается, но не подтверждено.
