...
Delegated offset и restore с offsetChanged запрещены на сервере и в БД. Signed snapshots содержат owner; expected/target должны совпадать с выбранным owner. Прямые grants записи lessons/notes не расширены, tutor_id существующего занятия неизменен. Отправка сообщений admin остаётся participant-based: назначения лично admin обязательны. Отдельный просмотр чужих диалогов добавлен в пакете 019 ниже.
...
## Пакет 019: просмотр чатов и исправления интерфейса

Иконка чатов перед шестерёнкой ставки в каталоге репетиторов открывает `/admin/chats?tutor=UUID`. Server Component и Server Actions проверяют active admin, UUID и выбранного active teacher; authenticated RPC повторно проверяют actor/owner без подмены JWT. Просмотр включает каталог, историю по 200 сообщений и приватные вложения. Он не отправляет сообщения и не меняет отметки прочтения репетитора. Signed URL выдаётся на 60 секунд только после проверки конкретного вложения в выбранном диалоге. Обычные участники сохраняют прежний доступ через RLS. Обновление видимой страницы происходит раз в 5 секунд, предыдущие страницы сохраняются в памяти.

Панель редактора вычисляет активные стили из текущего выделения; выравнивание имеет максимум одно активное значение. Формулы с нижними пределами не обрезаются вертикальным скроллом. У GIF и Telegram animation скрыта файловая строка, MP4 имеет проигрыватель в сообщении и предпросмотр до отправки. Сервер определяет video MIME по сигнатуре файла.

МСК принимает целое число от −12 до +12 непосредственно при вводе; пустое поле или одиночный знак при завершении ввода возвращают сохранённое значение. Для нескольких выбранных занятий отключены «Заниматься с…», «Перенести…», «Личное». Зелёный и голубой удалены из ручной палитры; цвета состояний completed/transfer продолжают вычисляться по прежним правилам.

## Пакет 020: расширенный LaTeX и фон

[Настройки и запуск компилятора](latex-renderer.md) описывают границу KaTeX и полноценного TeX. Настройки сохраняет authenticated Server Action после requireRole(admin); закрытая SQL-таблица и RPC повторяют проверку active admin. Чтение разрешено активным профилям. Компиляция доступна tutor/admin через Server Action и отдельный HTTP-сервис; секрет подключения остаётся серверным. Каждое задание выполняется в одноразовом контейнере без сети, host mounts и секретов. Ни Route Handler, ни запуск TeX в процессе Next.js не добавлены.

Фон расписания позиционируется относительно всей dashboard-content, включая заголовок и поля раздела. Общий фон сайта состоит из трёх декоративных окружностей без перехвата указателя; reduced motion отключает их движение. Просмотр чата администратором получил заголовок каталога, отступ поиска и перенос длинного имени.

## Пакет 021: LaTeX без отдельного сервиса

Пакет 021 заменяет компилятор из 020 на [браузерное отображение](latex-renderer.md). Docker/HTTP-сервис удалён, пользовательский код не отправляется внешним системам. KaTeX обслуживает формулы/таблицы/макросы, локальный TikZJax WASM выполняется в одноразовом worker с timeout, SVG фильтруется и показывается через img. Asymptote ограничен явно описанной 2D-грамматикой без eval/import/произвольных программ. Удалены поле преамбулы и ручное подключение произвольных системных TeX-пакетов; загрузка .sty разрешает только библиотечные определения команд. Исторические поля RPC 020 сохранены для совместимости; Server Action при сохранении очищает preamble.

Общий фон: SVG-контуры, 12 частиц, медленная анимация и небольшое смещение за указателем. Pointer events не перехватываются, обработчик обновляет стили через requestAnimationFrame только для точного указателя. При hidden document и reduced motion анимация останавливается; на мобильных экранах частиц меньше. Градиенты и glow не используются.
