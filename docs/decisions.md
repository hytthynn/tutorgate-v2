...
## ADR-003 — Soft-delete предметов
Удаление означает is_active=false. Исторические связи остаются. Новые назначения не используют неактивный предмет.
...
Первоначальный MVP оставлял расписание заглушкой. `TutorGate_Schedule_TZ.md` расширяет scope: добавлены lessons, private notes и пользовательский МСК-сдвиг. Availability, повторения и уведомления по-прежнему вне scope. Admin calendar показывает только собственные занятия; общая статистика может читать все занятия под RLS.

Фиксированный UTC offset (UTC+3+сдвиг) используется всеми чистыми функциями времени. `starts_at` — timestamptz; `ends_at` всегда вычисляется триггером. Два GiST exclusion constraints запрещают пересечения по tutor/student на полуоткрытых интервалах. Notes физически отделены от lessons и не выдаются ученику. Authenticated SECURITY INVOKER RPC сохраняет lesson+note атомарно; service role не используется.
