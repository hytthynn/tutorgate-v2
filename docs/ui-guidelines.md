...
Чат занимает оставшуюся высоту viewport и полную доступную ширину; прокрутка ограничена историей, списком диалогов и длинным вводом. Rich editor отображает стили непосредственно в contenteditable, сохраняет выделение при toolbar actions и безопасно разбирает вставку через whitelist DOM. Отдельного пункта «Ссылка» нет. Текстовые HTTP(S)/www ссылки в сообщениях кликабельны, опасные схемы запрещены. Кнопка «Обозначения цветов» — иконка палитры с доступной подписью рядом с биндами.

Сумма всех файлов одного сообщения ограничена 10 МБ в UI, Server Action и БД. Правило 014 «до 10 файлов по 10 МБ» заменено: количество по-прежнему не более 10, но бюджет общий. Снятие последнего назначения репетитора удаляет его чат и вложения у ученика.
...
## Уточнения интерфейса 016

Редактор явно применяет текущий набор стилей к новым символам, включая пустой набор после выключения кнопки. Полоса вложений использует тонкий скролл в цветах темы. Подсказка «Обозначения» использует тот же компонент, что «Бинды». Две служебные подписи про Telegram в чате удалены.

Звук входящего сообщения создаётся Web Audio без внешних ресурсов. Браузер разрешает его после взаимодействия с сайтом; начальная история и собственные сообщения не звучат. Polling сохраняет действующий режим активной вкладки. В чате сравниваются ID входящих сообщений, в других разделах — рост непрочитанного счётчика; повторное чтение не должно повторять звук.

Заявки получили поиск по имени и Telegram (replace, debounce 300 мс, сброс страницы с сохранением роли и статуса). Карточки компактнее, сведения сохраняются, на узких экранах сетка перестраивается.
