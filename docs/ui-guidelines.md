...
Общий Select/Combobox центрирует popup относительно trigger и ограничивает его краями viewport. Текстовый wordmark — TutorGate без точки. Поддержка доступна в desktop/sidebar, mobile menu и публичном header.

Composer поддерживает toolbar форматирования выделенного текста, shortcuts B/I/U, безопасный paste и предпросмотр; выбор файлов и drop используют одну очередь, файл можно удалить до отправки. Общий лимит — 10 файлов по 10 МБ. Полное удаление подтверждается кнопкой «Удалить аккаунт полностью» с явным предупреждением о потере расписания, статистики, переписки и файлов.
