...
С пакета 021 таблица и RPC 020 сохраняются для совместимости. Поле preamble не показывается и обнуляется новым Server Action; загруженные .sty проходят дополнительную проверку допустимых определений команд. Пакеты не устанавливаются в ОС, [рендеринг выполняется браузером](latex-renderer.md). Новой миграции нет.
